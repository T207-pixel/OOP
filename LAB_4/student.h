...
public:
	...
	virtual void print_in_file(ofstream& fout) {
		fout << "Surname: ;" << surname<< endl;
		fout << "Initials: ;" << initials << endl;
		fout << "Group: ;" << group << endl;
		fout << "cathedra_id: ;" << cathedra_id << endl;
	}
	...
	template<typename T>
	T getValueLoop(const string& err_message, const string& err_message2) { 
		T value;
